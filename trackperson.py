...
def extract_persons(img,bbox,count,num, identities=None):
    ...
    for i,box in enumerate(bbox):
        ...
        #calculate new xy coords to match chosen output size
        newx1 = center_x - outputdim[0]/2
        newy1 = center_y - outputdim[1]/2
        newx2 = center_x + outputdim[0]/2
        newy2 = center_y + outputdim[1]/2
        ...
        #print(f"X: ({x1},{x2})",f"Y: ({y1},{y2})",f"Size ({lenx},{leny})",f"nX: ({newx1},{newx2})",f"nY: ({newy1},{newy2})")

        # box text and bars
        id = int(identities[i]) if identities is not None else 0
        detection = img[newy1:newy2,newx1:newx2]
        #images[0:400,200*i:200*(i+1)] = detection


    cv2.imwrite("/content/res/img_"+ str(count).zfill(4) + ".jpg",images)
    return images
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    if len(outputs) > 0:
                        ...
                        extract_persons(im0, bbox_xyxy, imgcount, identities)
